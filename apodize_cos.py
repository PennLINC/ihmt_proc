...
## build cosine kernel
wx = np.array(
    np.cos((np.arange(img.shape[0]) - np.floor(img.shape[0] / 2)) * np.pi / img.shape[0])
)
wy = np.array(
    np.cos((np.arange(img.shape[1]) - np.floor(img.shape[1] / 2)) * np.pi / img.shape[1])
)
wz = np.array(
    np.cos((np.arange(img.shape[2]) - np.floor(img.shape[2] / 2)) * np.pi / img.shape[2])
)
...
## process fft & apodize
img_apo = np.zeros(
    (img.shape[0] * interpfac, img.shape[1] * interpfac, img.shape[2] * interpfac, img.shape[3])
)
img_data = img.dataobj
for ii in range(img.shape[3]):
    img_data_tmp = img_data[:, :, :, ii]
    img_data_fft = np.fft.fftshift(np.fft.fftn(img_data_tmp)) / (
