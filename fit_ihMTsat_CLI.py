...
def main():
    ## parse arguments
    text_description = (
        'Compute ihMT and MT saturation maps [1] from an ihMT-prepared experiment [2]. Outputs are in percentage unit.\
                        \nNotes: \
    ...
    parser = argparse.ArgumentParser(
        description=text_description, formatter_class=RawTextHelpFormatter
    )
    ...
    parser.add_argument(
        '--MTdsat', nargs='?', help='Output MTsat from dual-offset images NIfTI path'
    )
    parser.add_argument(
        '--MTssat', nargs='?', help='Output MTsat from single-offset images NIfTI path'
    )
    parser.add_argument(
        '--ihMTsatB1sq', nargs='?', help='Output ihMTsat image normalized by squared B1 NIfTI path'
    )
    parser.add_argument(
        '--MTdsatB1sq',
        nargs='?',
        help='Output MTsat from single-offset images normalized by squared B1 NIfTI path',
    )
    parser.add_argument(
        '--MTssatB1sq',
        nargs='?',
        help='Output MTsat from dual-offset images normalized by squared B1 NIfTI path',
    )
    ...
    parser.add_argument(
        '--R',
        nargs='?',
        help='Reference indices in 4D input (comma-separated integers; default: 1)',
    )
    parser.add_argument(
        '--S',
        nargs='?',
        help='Single-offset indices in 4D input (comma-separated integers; default: 2,4,...,N-1)',
    )
    parser.add_argument(
        '--D',
        nargs='?',
        help='Dual-offset indices in 4D input (comma-separated integers; default: 3,5,...,N)',
    )
    parser.add_argument(
        '--xtol',
        nargs='?',
    ...
    if len(args.ihMTparx) == 5:
        ...
        ihMTparx = ihMTparx_NT(
            dt=float(args.ihMTparx[0]) * 1e-3,
        ...
        parser.error(
            f'Wrong amount of ihMT preparation parameters (ihMTparx \
                             --- expected 3 (ihMT-GRE) or 5 (ihMT-RAGE), found {len(args.ihMTparx)})'
        )

    args.TFLparx = args.TFLparx.split(',')
    if len(args.TFLparx) != 4:
        parser.error(
            f'Wrong amount of sequence/readout parameters (TFLparx \
                         --- expected 4, found {len(args.TFLparx)})'
        )
    TFLparx_NT = collections.namedtuple('TFLparx_NT', 'subTR FA Npart TR')
    TFLparx = TFLparx_NT(
        subTR=float(args.TFLparx[0]) * 1e-3,
    ...
    if not os.path.isfile(ihMT_in_niipath) or len(nibabel.load(ihMT_in_niipath).shape) != 4:
        parser.error(f'Volume {ihMT_in_niipath} does not exist or is not 4D')
    print('ihMT provided volume exist')

    # check T1 map
    if not os.path.isfile(T1map_in_niipath):
        parser.error(f'T1 map volume {T1map_in_niipath} does not exist')
    ...
    if args.B1 is None:
        ...
        parser.error(f'B1 map volume {B1_in_niipath} does not exist')
    else:
    ...
    if args.mask is None:
        ...
        parser.error(f'Mask map volume {mask_in_niipath} does not exist')
    else:
    ...
    E1_subTR = numpy.exp(
        numpy.divide(
            -TFLparx.subTR, T1_data, out=numpy.ones(T1_data.shape, dtype=float), where=T1_data != 0
        )
    )
    ...
    if EXP_RAGE_GRE == 'RAGE':
        E1_ihMT = numpy.exp(
            numpy.divide(
                ...
                T1_data,
                out=numpy.ones(T1_data.shape, dtype=float),
                where=T1_data != 0,
            )
        )
        E1_RD = numpy.exp(
    ...
    xData_MT0 = [*zip(numpy.hstack((E1_ihMT, E1_subTR, E1_RD, cosFA_RO, Npart)), strict=False)]
    ...
    E1_dt = numpy.exp(
        numpy.divide(
            -ihMTparx.dt, T1_data, out=numpy.ones(T1_data.shape, dtype=float), where=T1_data != 0
        )
    )
    if EXP_RAGE_GRE == 'RAGE':
    ...
    MTs_iterable = [*zip(xData, MTs_yData, EXP_RAGE_GRE, xtolVal, strict=False)]
    MTd_iterable = [*zip(xData, MTd_yData, EXP_RAGE_GRE, xtolVal, strict=False)]
    ...
    if args.ihMTsatB1sq is not None and args.B1 is not None:
        ...
        ihMTsatB1sq_map = numpy.divide(
            ihMTsat_map,
            B1_map**2,
            out=numpy.zeros(ihMTsat_map.shape, dtype=float),
            where=B1_map != 0,
        )
    ...
    if args.MTdsatB1sq is not None and args.B1 is not None:
        ...
        MTdsatB1sq_map = numpy.divide(
            MTdsat_map,
            B1_map**2,
            out=numpy.zeros(MTdsat_map.shape, dtype=float),
            where=B1_map != 0,
        )
    ...
    if args.MTssatB1sq is not None and args.B1 is not None:
        ...
        MTssatB1sq_map = numpy.divide(
            MTssat_map,
            B1_map**2,
            out=numpy.zeros(MTssat_map.shape, dtype=float),
            where=B1_map != 0,
        )
...
def fit_MTsat_brentq(xData, yData, EXP_RAGE_GRE, xtolVal):
    ...
    try:
        if EXP_RAGE_GRE == 'RAGE':
            x0 = scipy.optimize.brentq(func_MTsat_RAGE_root, 0, 0.3, (xData, yData), xtol=xtolVal)
        else:
            x0 = scipy.optimize.brentq(func_MTsat_GRE_root, 0, 0.3, (xData, yData), xtol=xtolVal)
        return x0
    except:
        return 0
